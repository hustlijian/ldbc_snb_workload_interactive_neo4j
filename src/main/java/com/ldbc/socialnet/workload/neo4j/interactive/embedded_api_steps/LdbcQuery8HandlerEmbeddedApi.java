...
public class LdbcQuery8HandlerEmbeddedApi extends OperationHandler<LdbcQuery8> {
    @Override
    protected OperationResultReport executeOperation(LdbcQuery8 operation) throws DbException {
        ...
        try (Transaction tx = db.beginTx()) {
            result = ImmutableList.copyOf(new Neo4jQuery8EmbeddedApi(traversers).execute(db, operation));
            tx.success();
        } catch (Exception e) {
