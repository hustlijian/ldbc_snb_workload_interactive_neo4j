...
import com.ldbc.socialnet.workload.neo4j.load.LdbcSnbNeo4jImporter;
...
public class IntegrationTest {
    ...
    @Test
    public void shouldRunEmbeddedStepsTransactionalWorkloadWithoutThrowingException() throws ClientException, IOException {
        ...
        userParams.put(Neo4jDb.DB_PATH_KEY, dbDir.getAbsolutePath());
    ...
    @Test
    public void shouldRunEmbeddedCypherTransactionalWorkloadWithoutThrowingException() throws ClientException, IOException {
        File dbDir = testFolder.newFile();
        buildGraph(dbDir.getAbsolutePath());
        File resultsFile = testFolder.newFile();
        assertThat(resultsFile.length() == 0, is(true));
    ...
    @Test
    public void shouldLoadParametersFromFileInsteadOfCommandLine() throws IOException, DriverConfigurationException, ClientException {
        File dbDir = testFolder.newFile();
        buildGraph(dbDir.getAbsolutePath());
        File resultsFile = testFolder.newFile();
        assertThat(resultsFile.length() == 0, is(true));
        ...
        Properties neo4jDbProperties = new Properties();
        neo4jDbProperties.load(new FileInputStream(TestUtils.getResource("/neo4j_ldbc_socnet_interactive_test.properties")));
        Map<String, String> neo4jDbConfiguration = MapUtils.propertiesToMap(neo4jDbProperties);
        neo4jDbConfiguration.put(Neo4jDb.CONFIG_PATH_KEY, TestUtils.getResource("/neo4j_import_dev.properties").getAbsolutePath());
        ...
        additionalParameters.put(ConsoleAndFileDriverConfiguration.TOLERATED_EXECUTION_DELAY_ARG, Long.toString(Duration.fromMinutes(1).asMilli()));
